...
interface SeatingMapHeaderProps {
  ...
  venueFile: string;
  onSwitchVenue: (venueFile: string) => void;
}
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 sm:mb-4 md:mb-6 pb-3 sm:pb-4 border-b-2 border-slate-200 dark:border-slate-700 gap-3 sm:gap-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <select
          ...
          className="px-3 py-2 text-xs sm:text-sm rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-600 focus:outline-none focus:ring-2 focus:ring-primary-500 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
        >
          <option value="venue.json">Small Venue (68 seats)</option>
          <option value="venue-15k.json">Large Venue (15k seats)</option>
        </select>
        <button
